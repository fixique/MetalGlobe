...
struct SphereGenerator {
    ...
    static func sphereWithRadius(_ radius: Float32, stacks: Int, slices: Int, device: MTLDevice) -> (MTLBuffer, MTLBuffer) {
        let vertices = getVertices(radius: radius, stacks: stacks, slices: slices)
        let indices = getIndices(stacks: stacks, slices: slices)
    ...
    private static func getVertices(radius: Float32, stacks: Int, slices: Int) -> [Vertex] {
    ...
    private static func getIndices(stacks: Int, slices: Int) -> [UInt16] {
        var indices = [UInt16]()
